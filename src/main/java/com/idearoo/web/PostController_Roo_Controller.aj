...
privileged aspect PostController_Roo_Controller {
    ...
    public String PostController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        if (page != null || size != null) {
            ...
            uiModel.addAttribute("posts", Post.findPostEntries(page == null ? 0 : (page.intValue() - 1) * sizeNo, sizeNo));
    ...
    public Collection<Post> PostController.populatePosts() {
        return Post.findAllPosts();
    }
    
    void PostController.addDateTimeFormatPatterns(Model uiModel) {
        uiModel.addAttribute("post_postdate_date_format", DateTimeFormat.patternForStyle("S-", LocaleContextHolder.getLocale()));
    }
